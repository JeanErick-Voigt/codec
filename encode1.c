...
int main(int argc, char *argv[])
{
	...
	switch(type)
	{
		...
		case 0x11:
			...
			char * buffer1;
			fgets(line, 100, fp);
			sscanf(line, "%s : %[^\n]s/%s", garbage, buffer, buffer1);
			uint8_t hp1 = ((atoi(buffer) & 0xFF0000) >> 16);
			uint8_t hp2 = ((atoi(buffer) & 0xFF00) >> 8);
			uint8_t hp3 = (atoi(buffer) & 0xFF);
			status.hitPoints[0] = hp1;
			status.hitPoints[1] = hp2;
			status.hitPoints[2] = hp3;
			uint8_t mhp1 = ((atoi(buffer) & 0xFF0000) >> 16);
